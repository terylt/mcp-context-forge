...
class ConstraintBundle:
    ...
    finish_no_later_than: Optional[str] = None
...
def generate_work_breakdown(
    ...
    constraint_bundle = ConstraintBundle(
        ...
        budget_limit=float(constraints["budget_limit"]) if constraints and "budget_limit" in constraints else None,
    )
...
def critical_path_analysis(schedule: ScheduleModel) -> CriticalPathResult:
    ...
    project_duration = max((task.earliest_finish or 0.0) for task in tasks.values()) if tasks else 0.0
    ...
    for task_id in reversed(order):
        ...
        if not any(task_id in tasks[child].dependencies for child in tasks):
            ...
            lf = min(latest[child] - tasks[child].duration_days for child in tasks if task_id in tasks[child].dependencies)
        latest[task_id] = lf
        task.latest_finish = lf
        task.latest_start = lf - task.duration_days
        task.slack = (task.latest_start - task.earliest_start) if task.earliest_start is not None else 0.0
...
def gantt_artifacts(schedule: ScheduleModel, project_start: Optional[str]) -> DiagramArtifact:
...
def schedule_optimizer(schedule: ScheduleModel) -> ScheduleModel:
    ...
    for task in schedule.tasks:
        if task.id == longest_task.id and task.duration_days > 3:
            optimized_tasks.append(task.model_copy(update={"duration_days": task.duration_days * 0.9}))
        else:
