...
server = Server("code-splitter-server")


class SplitCodeRequest(BaseModel):
    """Request to split code."""
    code: str = Field(..., description="Source code to split")
    language: str = Field("python", description="Programming language")
    split_level: str = Field("function", description="Split level (function, class, method, all)")
    include_metadata: bool = Field(True, description="Include metadata about code segments")
    preserve_comments: bool = Field(True, description="Preserve comments in output")
    min_lines: int = Field(5, description="Minimum lines per segment", ge=1)
...
class AnalyzeCodeRequest(BaseModel):
    ...
    include_dependencies: bool = Field(True, description="Include import/dependency analysis")


class ExtractFunctionsRequest(BaseModel):
    """Request to extract functions from code."""
...
class ExtractClassesRequest(BaseModel):
    """Request to extract classes from code."""
    code: str = Field(..., description="Source code")
    language: str = Field("python", description="Programming language")
...
class CodeSplitter:
    ...
    def _extract_functions(self, tree: ast.AST, lines: List[str], include_metadata: bool) -> List[Dict[str, Any]]:
    ...
    def _extract_classes(self, tree: ast.AST, lines: List[str], include_metadata: bool) -> List[Dict[str, Any]]:
    ...
    def _extract_methods(self, tree: ast.AST, lines: List[str], include_metadata: bool) -> List[Dict[str, Any]]:
                    ...
                    if isinstance(method_node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                        ...
                        if include_metadata:
                            method_info.update({
                                ...
                                "decorators": [ast.unparse(dec) for dec in method_node.decorator_list],
                                "docstring": ast.get_docstring(method_node),
                                "is_async": isinstance(method_node, ast.AsyncFunctionDef),
                                "is_property": any("property" in ast.unparse(dec) for dec in method_node.decorator_list),
                                "is_static": any("staticmethod" in ast.unparse(dec) for dec in method_node.decorator_list),
                                "is_class_method": any("classmethod" in ast.unparse(dec) for dec in method_node.decorator_list)
                            })
    ...
    def _extract_imports(self, tree: ast.AST, lines: List[str], include_metadata: bool) -> List[Dict[str, Any]]:
            ...
            if stripped.startswith('#'):
                ...
                if current_comment:
                    ...
                    if include_metadata:
                        ...
                        comment_info["content"] = '\n'.join([line.strip().lstrip('#').strip() for line in current_comment])
            ...
            for node in ast.walk(tree):
                if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                    ...
                    if include_decorators:
                        function_info["decorators"] = [ast.unparse(dec) for dec in node.decorator_list]
                        ...
                        for method_node in node.body:
                            if isinstance(method_node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                                methods.append({
                                    ...
                                    "is_async": isinstance(method_node, ast.AsyncFunctionDef),
                                    "arguments": [arg.arg for arg in method_node.args.args],
                                    "line_range": [method_node.lineno, self._find_node_end_line(method_node, lines) + 1]
                                })
...
# Initialize splitter (conditionally for testing)
try:
...
async def handle_list_tools() -> list[Tool]:
    """List available code splitting tools."""
    return [
        Tool(
            name="split_code",
            description="Split code into logical segments using AST analysis",
            inputSchema={
                "type": "object",
                "properties": {
                    "code": {"type": "string", "description": "Source code to split"},
                    "language": {
                        "type": "string",
                        "enum": ["python"],
                    ...
                    "split_level": {
                        "type": "string",
                        "enum": ["function", "class", "method", "import", "all"],
                    ...
                    "min_lines": {
                        "type": "integer",
                        "description": "Minimum lines per segment",
                        "default": 5,
                        "minimum": 1
                    }
                },
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "include_docstrings": {
                        "type": "boolean",
                        "description": "Include function docstrings",
                        "default": True
                    },
                    "include_decorators": {
                        "type": "boolean",
                        "description": "Include function decorators",
                        "default": True
                    }
                },
                "required": ["code"]
        ...
        Tool(
            ...
            description="Extract class definitions from code",
            inputSchema={
                "type": "object",
                "properties": {
                    "code": {"type": "string", "description": "Source code"},
                    ...
                    "include_inheritance": {
                        "type": "boolean",
                        "description": "Include inheritance information",
                        "default": True
                    }
                },
                "required": ["code"]
...
async def handle_call_tool(name: str, arguments: dict[str, Any]) -> Sequence[TextContent | ImageContent | EmbeddedResource]:
    ...
    try:
        if splitter is None:
            result = {"success": False, "error": "Code splitter not available"}
        elif name == "split_code":
            request = SplitCodeRequest(**arguments)
            result = splitter.split_python_code(
                code=request.code,
            ...
            request = AnalyzeCodeRequest(**arguments)
            result = splitter.analyze_code_structure(
                code=request.code,
            ...
            request = ExtractFunctionsRequest(**arguments)
            result = splitter.extract_functions_only(
                code=request.code,
                language=request.language,
                include_docstrings=request.include_docstrings,
                include_decorators=request.include_decorators
            )

        elif name == "extract_classes":
            request = ExtractClassesRequest(**arguments)
            result = splitter.extract_classes_only(
                code=request.code,
                language=request.language,
                include_methods=request.include_methods,
                include_inheritance=request.include_inheritance
            )
...
async def main():
    ...
    logger.info("Starting Code Splitter MCP Server...")
    ...
    async with stdio_server() as (read_stream, write_stream):
        ...
        await server.run(
            ...
            InitializationOptions(
                server_name="code-splitter-server",
