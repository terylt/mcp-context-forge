...
server = Server("csv-pandas-chat-server")
...
class ChatWithCSVRequest(BaseModel):
    ...
    file_url: Optional[HttpUrl] = Field(None, description="URL to CSV or XLSX file")
...
class GetCSVInfoRequest(BaseModel):
    """Request to get CSV information."""
    csv_content: Optional[str] = Field(None, description="CSV content as string")
    file_url: Optional[HttpUrl] = Field(None, description="URL to CSV or XLSX file")
    file_path: Optional[str] = Field(None, description="Path to local CSV file")
...
class AnalyzeCSVRequest(BaseModel):
    """Request to analyze CSV data structure."""
    csv_content: Optional[str] = Field(None, description="CSV content as string")
    file_url: Optional[HttpUrl] = Field(None, description="URL to CSV or XLSX file")
    file_path: Optional[str] = Field(None, description="Path to local CSV file")
    analysis_type: str = Field("basic", description="Type of analysis (basic, detailed, statistical)")
...
class CSVProcessor:
    ...
    def sanitize_code(self, code: str) -> str:
        ...
        blocklist = [
            ...
            r'\bfor\s+.*\s+in\s+.*:\s*$',  # Potentially infinite for loops without clear end
        ]
        ...
        # Allowlist of safe pandas and numpy operations
        safe_patterns = [
            r'\bdf\.',
            r'\bpd\.',
            r'\bnp\.',
...
"""
    ...
    async def chat_with_csv(
        ...
        """Process a chat query against CSV data."""
        ...
        try:
            ...
            df = await self.load_dataframe(csv_content, file_url, file_path)
            logger.info(f"Loaded dataframe with shape: {df.shape}")

            # Prepare data for LLM
            df_head = df.head(5).to_markdown()
            column_info = self.extract_column_info(df)

            # Generate code using OpenAI
            llm_response = await self._generate_code_with_openai(
                df_head, column_info, sanitized_query, openai_api_key, model
            )

            # Execute the generated code
            if "code" in llm_response and llm_response["code"]:
                code = self.sanitize_code(llm_response["code"])
                code = self.fix_syntax_errors(code)

                result = await self.execute_code_with_timeout(code, df)
    ...
    async def _generate_code_with_openai(
        ...
        # Use OpenAI API (you may need to install openai package)
        try:
            import openai
...
async def handle_list_tools() -> list[Tool]:
    """List available CSV chat tools."""
    return [
        Tool(
            name="chat_with_csv",
            description="Chat with CSV data using natural language queries",
            inputSchema={
                "type": "object",
                "properties": {
                    "query": {
                    ...
                    "csv_content": {
                        "type": "string",
                        "description": "CSV content as string (optional)"
                    },
                    "file_url": {
                        "type": "string",
                        "description": "URL to CSV or XLSX file (optional)"
                    },
                    "file_path": {
                        "type": "string",
                        "description": "Path to local CSV file (optional)"
                    },
                    "openai_api_key": {
                        "type": "string",
        ...
        Tool(
            name="get_csv_info",
            description="Get comprehensive information about CSV data structure",
            inputSchema={
                "type": "object",
                "properties": {
                    "csv_content": {
        ...
        Tool(
            name="analyze_csv",
            description="Perform automated analysis of CSV data",
            inputSchema={
                "type": "object",
                "properties": {
                    "csv_content": {
...
async def handle_call_tool(name: str, arguments: dict[str, Any]) -> Sequence[TextContent | ImageContent | EmbeddedResource]:
    ...
    try:
        if processor is None:
            result = {"success": False, "error": "CSV processor not available"}
        elif name == "chat_with_csv":
            request = ChatWithCSVRequest(**arguments)
            result = await processor.chat_with_csv(
                query=request.query,
            ...
            request = GetCSVInfoRequest(**arguments)
            result = await processor.get_csv_info(
                csv_content=request.csv_content,
                file_url=str(request.file_url) if request.file_url else None,
                file_path=request.file_path
            )
            ...
            request = AnalyzeCSVRequest(**arguments)
...
async def main():
    ...
    logger.info("Starting CSV Pandas Chat MCP Server...")
    ...
    async with stdio_server() as (read_stream, write_stream):
        ...
        await server.run(
            ...
            InitializationOptions(
                server_name="csv-pandas-chat-server",
